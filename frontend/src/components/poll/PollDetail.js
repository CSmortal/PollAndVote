...
export default function PollDetail() {
    ...
    useEffect(() => {
        async function getPollInDetail() {
            return await fetch(`http://localhost:8080/api/getPoll?pollId=${pollId}&userId=${userId}`, {
                headers: {
    ...
    async function handleEndPoll() {
        const response = await fetch(`http://localhost:8080/api/endPoll/${pollId}`, {
            method: "PUT",
            headers: {
                "Authorization" : localStorage.getItem("token")
            }
        ...
        if (response) {
            ...
            const finalPollResult = await fetch(`http://localhost:8080/api/getPoll?pollId=${pollId}&userId=${userId}`, {
                headers: {
                    "Authorization" : localStorage.getItem("token")
                }
            }).then(res => res.json())
